...
def CHORD_antenna_positions(N_NS=N_NS,N_EW=N_EW,offset_deg=1.75,                       # basics required to specify a CHORD-like array
                            num_antennas_to_perturb=0,antenna_perturbation_sigma=1e-3, # controls for examining the effect of misplaced antennas
                            num_weights_to_perturb=0,weight_perturbation_sigma=1e-3):  # controls for 0th-order modelling of primary beam differences
    N_ant=N_NS*N_EW
    ...
    if (num_antennas_to_perturb>0):
        indices_ants=np.random.randint(0,N_ant,size=num_antennas_to_perturb) # indices of antennas to perturb
        x_perturbations=np.zeros((N_ant,))
        x_perturbations[indices_ants]=np.random.normal(loc=0.,scale=antenna_perturbation_sigma/np.sqrt(3),size=np.insert(num_antennas_to_perturb,0,1))
        y_perturbations=np.zeros((N_ant,))
        y_perturbations[indices_ants]=np.random.normal(loc=0.,scale=antenna_perturbation_sigma/np.sqrt(3),size=np.insert(num_antennas_to_perturb,0,1))
        z_perturbations=np.zeros((N_ant,))
        z_perturbations[indices_ants]=np.random.normal(loc=0.,scale=antenna_perturbation_sigma/np.sqrt(3),size=np.insert(num_antennas_to_perturb,0,1))
    ...
    if (num_weights_to_perturb>0):
        ...
        wt_perturbations[indices_wts]=np.random_normal(loc=0.,scale=weight_perturbation_sigma,size=np.insert(num_weights_to_perturb,0,1))
        ant_weights+=wt_perturbations
    else:
        indices_wts=None
    return antennas_ENU,ant_weights,[indices_ants,indices_wts]
...
for nu_obs in obs_freqs:
    ...
    axs[2,3].imshow(np.ma.masked_where(np.abs(ratio-1.)<tol,ratio),cmap="Reds") # not showing up the way I wanted to with > ...maybe I was mentally inverting "mask to keep" vs. "mask away"
