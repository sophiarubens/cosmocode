...
def higher_dim_conv(f,g): # NEED TO ZERO-PAD THE ARRAYS ... in all directions, I think?!
    ...
    # print("result.shape=",result_p.shape)
    result=result_p[:a,:b].real
    # result=np.flip(np.flip(result,0),1) # LOOKS LIKE IT NEEDS TO HAPPEN, BUT HOW TO MOTIVATE THIS MATHEMATICALLY??
    return result.real
...
plt.tight_layout()
plt.savefig("check_fft_cheat_mult.png")
plt.show()