...
def CHORD_antenna_positions(N_NS=N_NS,N_EW=N_EW,offset_deg=1.75,                    # basics required to specify a CHORD-like array
    ...
    if (num_pbs_to_perturb>0):
        indices_pbs=np.random.randint(0,N_ant,size=num_pbs_to_perturb) # indices of antenna pbs to perturb (not necessarily the same as the indices of the antenna positions to perturb, because the two perturbations should be able to be called with different numbers of antennas, and it would be nice for the independence to still hold when the numbers of perturbed elements are the same)
        ...
        indices_pbs=None
    return antennas_ENU,ant_pb_frac_widths,[indices_ants,indices_pbs]
...
def calc_baselines_xyz(antennas_ENU,antenna_pbs,N_NS=N_NS,N_EW=N_EW):
    N_ant=N_NS*N_EW
    N_bl=N_ant*(N_ant-1)//2
    
    # calculate baselines in ENU
    baselines_ENU=np.zeros((N_bl,3))
    baseline_pbs=np.zeros(N_bl)
    for i in range(N_NS):
        ...
        for j in range(N_EW):
            ...
            baselines_ENU[-(k+1),:]= -baseline_ij # also accumulate a copy for the version where the other antenna is the referent
            baseline_pbs[k]=antenna_pbs[i]*antenna_pbs[j] # NO!!!!! (update with notebook math)
    ...
    return baselines_xyz,baseline_pbs
...
N_pbs_to_pert=100
prb_pert=1e-2
antennas_ENU_fidu,antenna_pbs_fidu,_=                  CHORD_antenna_positions()
antennas_ENU_antp,antenna_pbs_antp,[indices_antp_a,_]= CHORD_antenna_positions(num_antpos_to_perturb=N_ant_to_pert, antpos_perturbation_sigma=ant_pert)
antennas_ENU_prbp,antenna_pbs_prbp,[_,indices_prbp_p]= CHORD_antenna_positions(num_pbs_to_perturb=N_pbs_to_pert,pb_perturbation_sigma=prb_pert)
antennas_ENU_both,antenna_pbs_both,[indices_both_a,
...
baselines_xyz_fidu,baseline_pbs_fidu=calc_baselines_xyz(antennas_ENU_fidu,antenna_pbs_fidu,N_NS=N_NS,N_EW=N_EW)
baselines_xyz_antp,baseline_pbs_antp=calc_baselines_xyz(antennas_ENU_antp,antenna_pbs_antp,N_NS=N_NS,N_EW=N_EW)
baselines_xyz_prbp,baseline_pbs_prbp=calc_baselines_xyz(antennas_ENU_prbp,antenna_pbs_prbp,N_NS=N_NS,N_EW=N_EW)
baselines_xyz_both,baseline_pbs_both=calc_baselines_xyz(antennas_ENU_both,antenna_pbs_both,N_NS=N_NS,N_EW=N_EW)
t2=time.time()
...
uvw_synth_prbp=calc_rot_synth_uv(baselines_xyz_prbp,num_hrs=N_obs_hrs)
uvw_synth_both=calc_rot_synth_uv(baselines_xyz_both,num_hrs=N_obs_hrs)
t3=time.time()
...
for nu_obs in obs_freqs:
    ...
    for i in range(4):
        ...
        axs[i,3].set_xlabel("u ($\lambda$) pixel index")
        axs[i,3].set_ylabel("v ($\lambda$) pixel index")
    ...
    axs[1,0].scatter(antennas_ENU_antp[indices_antp_a,0],antennas_ENU_antp[indices_antp_a,1],s=dotsize,c="r")
    ...
    im=axs[1,5].imshow(ratio,cmap="Blues",origin="lower")
    ...
    axs[2,0].scatter(antennas_ENU_prbp[:,0],             antennas_ENU_prbp[:,1],             s=dotsize,c=antennas_ENU_prbp[:,2],cmap=trunc_Blues)
    axs[2,0].scatter(antennas_ENU_prbp[indices_prbp_p,0],antennas_ENU_prbp[indices_prbp_p,1],s=dotsize,c="tab:orange")
    ...
    im=axs[2,5].imshow(ratio,cmap="Blues",origin="lower")
    ...
    axs[3,0].scatter(antennas_ENU_both[:,0],           antennas_ENU_both[:,1],           s=dotsize,c=antennas_ENU_both[:,2],cmap=trunc_Blues)
    axs[3,0].scatter(antennas_ENU_both[indices_both_a,0],antennas_ENU_both[indices_both_a,1],s=dotsize,c="r")
    axs[3,0].scatter(antennas_ENU_both[indices_both_p,0],antennas_ENU_both[indices_both_p,1],s=dotsize,c="tab:orange")
    ...
    im=axs[3,5].imshow(ratio,cmap="Blues",origin="lower")
